...
from models.db import Model, db_session
...
@router.get("/")
async def get_models() -> Union[ResultSuccessClass[list[ModelResponse]], ResultErrorClass]:
    result = Model.query.all()
    result = [ModelResponse(id=model.id, 
                            name=model.name, 
                            size=convert_size(model.size), 
                            created_at=convert_datetime(model.created_at), 
                            updated_at=convert_datetime(model.updated_at)) for model in result]
...
@router.post("/new")
async def new_model(name: str = Form(...), model: UploadFile = File(...)) -> Union[ResultSuccessClass[ModelCreateResponse], ResultErrorClass]:
    model_id = str(uuid4())
